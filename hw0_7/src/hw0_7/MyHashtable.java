...
public class MyHashtable {
	...
	public boolean put(int key) {  
		...
		while(true) {
			...
			if(i>=m)  // 해시테이블 크기만큼 비어있는 공간을 찾지 못하면 삽입 실패
				return false;
			
			else if(table[j]==key) {   // 이미 있는 학번이면 삽입 실패
				return false;
			}
			
			else if(table[j]==EMPTY || table[j]==DELETED) {  //해시값 주소위치가 비어있으면 저장한다.
				table[j] = key;  // 키값 저장
				return true;
			}
	...
	private int search(int key) {
		...
		while(true) {
			...
			if(i>=m)  //테이블 크기만큼 될때까지 찾지 못하면 검색 실패
				return -1;

			else if(table[j]==EMPTY) {  //해시 테이블 공간이 비어있으면 검색 실패
				return -1;
			}
			
			else if(table[j]==key) {   // key값을 찾으면  
				return j;    //인덱스 리턴
			}
	...
	public boolean remove(int key) {  
		...
		while(true) {
			...
			if(i>=m)   // 해시 테이블 크기만큼 key값을 찾지 못하면 삭제 실패
				return false;
		
			else if(table[j]==key) {    //해시값 주소위치 원소가 key 값이면
